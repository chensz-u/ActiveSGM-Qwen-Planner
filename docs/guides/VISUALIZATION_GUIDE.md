...
### 方法 2：自定义参数运行
```bash
bash scripts/visualization/run_visualization.sh [scene] [exp] [gpus] [enable_vis]
```
...
```bash
bash scripts/visualization/run_visualization.sh office0 ActiveSem 0,1 1
```

**运行 office1 场景的 ActiveGS 配置：**
```bash
bash scripts/visualization/run_visualization.sh office1 ActiveGS 0,1 1
```

**仅使用 GPU 0：**
```bash
bash scripts/visualization/run_visualization.sh office0 ActiveSem 0 1
```

**禁用可视化窗口（仅保存结果）：**
```bash
bash scripts/visualization/run_visualization.sh office0 ActiveSem 0,1 0
```
