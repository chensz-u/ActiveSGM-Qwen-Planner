...
# Global-local Qwen log-only mode.
# This mode records Qwen's global-local judgment but does NOT change trajectory.
...
RESULT_DIR="results/Replica/office0/ActiveSem/run_qwen_global_local_logonly_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RESULT_DIR"
...
echo "===== START ActiveSGM Qwen Global-Local LOG-ONLY ====="
